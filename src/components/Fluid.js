...
class AggregateListener {
  ...
  add(observer, name, cb, useCapture) {
    observer.addEventListener(name, cb);
...
export default memo(({ randomSplashWeight }) => {
  const canvasRef = useRef();

  useEffect(() => {
    const fluid = FluidInit(canvasRef.current);

    let randomTimeoutId;
    if (randomSplashWeight) {
      const randomSplash = () => {
        fluid.multipleSplats(Math.floor(Math.random() * 10) + 4);

        randomTimeoutId = setTimeout(
          randomSplash,
          Math.ceil(Math.random() * randomSplashWeight) + 500,
        );
      };
