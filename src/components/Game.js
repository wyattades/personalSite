...
const HEIGHT = 600;
const SRC_DIR =
  'https://github.com/wyattades/personal-site/blob/master/src/sketches';
...
export default class Game extends React.Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      import('p5'),
      import(
        /* webpackChunkName: "sketches/[request]" */ `../sketches/${game.id}`
      ),
    ])
      .then(([p5, sketch]) => {
        this.setState(
          ...
          () => {
            ...
            const P5 = p5.default;
            this.sketch = new P5(
              (_p5) => sketch.default(_p5, w, HEIGHT, P5),
  ...
  render() {
    ...
    return (
      <AnimatedPage className="content">
        <Link className="goBack" to="/projects/games">
          ←
        </Link>
        <div>
          <h1>{game.title}</h1>
